...
class TestMCPToolIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_books_limit_validation(self, mock_get_client):
        ...
        mock_client.search_library.assert_called_with(
            query=None, limit=200, sort="title"  # Should cap at 200
        )
        
        # Test limit too low
        await list_books(limit=0)
        mock_client.search_library.assert_called_with(
            query=None, limit=1, sort="title"  # Should enforce minimum 1
        )
    ...
    async def test_search_books_default_fields(self, mock_get_client):
        ...
        result = await search_books(text="test")
...
class TestWorkflowIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_book_discovery_workflow(self, mock_get_client):
        ...
        programming_books = [
            {"id": 2, "title": "Python Guide", "authors": ["Coder"], "tags": ["programming"], "formats": ["PDF"]}
        ]
    ...
    async def test_series_exploration_workflow(self, mock_get_client):
        ...
        series_books = [
            {
                "id": 1, "title": "Foundation", "series": "Foundation Series", 
                "series_index": 1, "authors": ["Isaac Asimov"], "tags": ["sci-fi"], "formats": ["EPUB"]
            },
            {
                "id": 2, "title": "Foundation and Empire", "series": "Foundation Series",
                "series_index": 2, "authors": ["Isaac Asimov"], "tags": ["sci-fi"], "formats": ["EPUB"]
            }
        ]
