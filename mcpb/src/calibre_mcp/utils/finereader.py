...
class FineReaderCLI:
    ...
    def is_available() -> bool:
        """
        Check if FineReader CLI is available without instantiating the class.

        Checks in order:
        1. Environment variable FINEREADER_CLI_PATH
        2. System PATH (FineCmd.exe)
        3. Common installation paths (fallback only)

        Returns:
            True if FineReader CLI can be found, False otherwise
        """
        ...
        # Fallback: check common installation paths (lazy check - only if needed)
        ...
        if programfiles_x86:
            possible_paths.append(Path(programfiles_x86) / "ABBYY FineReader 15/FineCmd.exe")
        if programfiles:
            possible_paths.append(Path(programfiles) / "ABBYY FineReader 15/FineCmd.exe")
    ...
    def find_cli_path() -> Path | None:
        ...
        Returns:
            Path to FineCMD.exe if found, None otherwise
        """
        # Check environment variable first (user-specified path)
        env_path = os.getenv("FINEREADER_CLI_PATH")
        if env_path:
            path = Path(env_path)
            if path.exists():
        ...
        if which_result:
            ...
            logger.info(f"Found FineReader CLI in PATH: {path}")
            return path
        ...
        for path in possible_paths:
            if path.exists():
                logger.info(f"Found FineReader CLI at default location: {path}")
                return path
    ...
    def __init__(self, cli_path: str = None):
        ...
        if cli_path:
            ...
            if not self.cli_path.exists():
                raise FileNotFoundError(f"Specified FineReader CLI path not found: {cli_path}")
        else:
            found_path = self.find_cli_path()
            if not found_path:
                raise FileNotFoundError(
                    "FineReader CLI not found. "
                    "Set FINEREADER_CLI_PATH environment variable, "
                    "add FineCmd.exe to PATH, or install FineReader 15+."
                )
            self.cli_path = found_path
    ...
    async def process_document(
        ...
        try:
            ...
            raise OCRProcessingError(f"Unexpected error during OCR: {str(e)}") from e
    ...
    async def detect_language(self, input_path: str | Path) -> str:
        ...
        cmd = [
            ...
            str(input_path),
            "/lang",
            OCRLanguage.AUTO_DETECT.value,
            "/out",
            "NUL",  # No output needed for detection
            "/quit",
        ]
...
async def safe_ocr_process(
    ...
    for attempt in range(max_retries):
        try:
            result = await finereader.process_document(file_path, output_path, language=language)
    ...
    return {"success": False, "error": "Unknown error", "attempt": max_retries}
