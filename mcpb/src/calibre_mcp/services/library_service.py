...
class LibraryService(BaseService[Library, LibraryCreate, LibraryUpdate, LibraryResponse]):
    ...
    def get_all(
        ...
        with self._get_db_session() as session:
            ...
            query = query.outerjoin(
                book_count,
                Library.id == book_count.c.library_id
            ).outerjoin(
                author_count,
                Library.id == author_count.c.library_id
            )
            ...
            if search:
                ...
                query = query.filter(
                    or_(
                        ...
                        Library.path.ilike(search_term)
                    )
                )
            ...
            # Apply pagination
            libraries = query.options(
                joinedload(Library.books)
    ...
    def update(self, library_id: int, library_data: Union[LibraryUpdate, Dict[str, Any]]) -> Dict[str, Any]:
        """
