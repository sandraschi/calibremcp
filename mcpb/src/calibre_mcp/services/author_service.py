...
class AuthorService(BaseService[Author, AuthorCreate, AuthorUpdate, AuthorResponse]):
    ...
    def get_all(
        ...
        with self._get_db_session() as session:
            ...
            book_count = (
                session.query(Author.id.label("author_id"), func.count(Book.id).label("book_count"))
                .join(Author.books)
                .group_by(Author.id)
                .subquery()
            )
            ...
            query = query.outerjoin(book_count, Author.id == book_count.c.author_id)
            ...
            # Apply pagination
            authors = query.options(joinedload(Author.books)).offset(skip).limit(limit).all()
