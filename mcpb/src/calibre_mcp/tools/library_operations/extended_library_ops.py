...
class ExtendedLibraryOperations(MCPTool):
    ...
    async def analyze_library(self, library_path: str) -> Dict:
        ...
        stats.tag_cloud = dict(sorted(stats.tag_cloud.items(), key=lambda x: x[1], reverse=True))
    ...
    def _calculate_book_similarity(
        ...
        weights = {"title": 0.5, "authors": 0.4, "isbn": 0.9}
    ...
    async def backup_library(
        ...
        try:
            ...
            return {"error": f"Backup failed: {str(e)}", "success": False}
