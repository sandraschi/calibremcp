...
class AIEnhancementsTool(MCPTool):
    ...
    async def _init_ai_client(self) -> None:
        ...
        if not hasattr(self, '_ai_client'):
            try:
                ...
                logger.info("AI client initialized", extra={
                ...
                logger.error("Failed to initialize AI client", extra={
                    "service": "ai_enhancements",
    ...
    async def _generate_metadata(self, metadata, fields: List[str]) -> Dict:
        ...
        try:
            logger.info("Generating metadata with AI", extra={
                "service": "ai_enhancements",
            ...
            if response.status_code != 200:
                ...
                logger.error("AI service returned error", extra={
                    "service": "ai_enhancements",
            ...
            logger.info("Successfully generated metadata", extra={
                "service": "ai_enhancements",
            ...
            logger.error("Request to AI service failed", extra={
                "service": "ai_enhancements",
                "action": "generate_metadata_error",
                "error": str(e),
                "error_type": type(e).__name__
    ...
    async def _get_book_embedding(self, metadata) -> List[float]:
        ...
        try:
            logger.debug("Generating book embedding", extra={
                "service": "ai_enhancements",
            ...
            if response.status_code != 200:
                error_data = response.json()
                logger.error("Embedding API returned error", extra={
                    "service": "ai_enhancements",
                    "action": "generate_embedding_error",
                    "status_code": response.status_code,
                    "error": error_data
                })
                raise AIServiceError("Embedding API error", 
                                  status_code=response.status_code,
                                  details=error_data)
            ...
            logger.debug("Successfully generated embedding", extra={
                "service": "ai_enhancements",
