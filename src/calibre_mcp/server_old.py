...
from .logging_config import get_logger, log_operation, log_api_call, log_error
...
# Tool moved to tools/core/library_operations.py
    """
    ...
    try:
        ...
        log_error(logger, "list_books_unexpected", e)
        return LibrarySearchResponse(
            results=[],
            total_found=0,
            query_used=query,
...
async def get_book_details(book_id: int) -> BookDetailResponse:
    ...
    try:
        ...
        log_error(logger, "get_book_details", e, book_id=book_id)
        return BookDetailResponse(
            book_id=book_id,
            title=f"Error: {str(e)}",
            authors=[],
        ...
        log_error(logger, "get_book_details_unexpected", e, book_id=book_id)
        return BookDetailResponse(
            book_id=book_id,
...
async def search_books(
    ...
    try:
        ...
        log_error(logger, "search_books", e, query=query)
        return LibrarySearchResponse(
            results=[],
            total_found=0,
        ...
        log_error(logger, "search_books_unexpected", e, query=query)
        return LibrarySearchResponse(
            results=[],
            total_found=0,
...
async def test_calibre_connection() -> ConnectionTestResponse:
    ...
    try:
        ...
        log_error(logger, "calibre_connection_test", e)
        return ConnectionTestResponse(
            connected=False,
            response_time_ms=response_time_ms,
            error_message=str(e),
        ...
        log_error(logger, "connection_test_system_error", e)
        return ConnectionTestResponse(
            connected=False,
            response_time_ms=response_time_ms,
...
async def list_libraries() -> LibraryListResponse:
    ...
    try:
        ...
        if library_path and os.path.exists(library_path):
            ...
            if os.path.exists(metadata_db):
                try:
                    ...
                    log_operation(logger, "metadata_db_warning", level="WARNING", 
                                 error=str(e), library_path=str(library_path))
        ...
        log_error(logger, "list_libraries", e)
        return LibraryListResponse(
            libraries=[],
            current_library=current_library,
            total_libraries=0
...
async def switch_library(library_name: str):
    ...
    try:
        ...
        log_operation(logger, "library_switch", level="INFO", 
                     old_library=old_library, new_library=current_library, status="success")
        
        # Return updated library list to show the current selection
        return await list_libraries()
        
    except Exception as e:
        log_error(logger, "switch_library", e, library_name=library_name)
        return await list_libraries()
...
async def get_library_stats(library_name: Optional[str] = None) -> LibraryStatsResponse:
    ...
    try:
        ...
        log_error(logger, "get_library_stats", e, library_name=target_library)
        return LibraryStatsResponse(
            library_name=target_library,
            total_books=0,
            total_authors=0,
...
async def cross_library_search(
    ...
    try:
        ...
        log_error(logger, "cross_library_search", e)
        return LibrarySearchResponse(
            results=[],
            total_found=0,
            query_used=query,
...
async def get_tag_statistics() -> TagStatsResponse:
    ...
    try:
        ...
        log_error(logger, "analyze_tags", e)
        return TagStatsResponse(
            total_tags=0,
            most_used_tags=[],
            unused_tags=[],
...
async def find_duplicate_books() -> DuplicatesResponse:
    ...
    try:
        ...
        log_error(logger, "find_duplicates", e)
        return DuplicatesResponse(
            duplicates_found=[],
...
async def get_series_analysis() -> SeriesAnalysisResponse:
    ...
    try:
        ...
        log_error(logger, "analyze_series", e)
        return SeriesAnalysisResponse(
            series_analyzed=[],
            incomplete_series=[],
            complete_series=[],
...
async def analyze_library_health() -> LibraryHealthResponse:
    ...
    try:
        ...
        log_error(logger, "analyze_library_health", e)
        return LibraryHealthResponse(
...
async def unread_priority_list() -> UnreadPriorityResponse:
    ...
    try:
        ...
        log_error(logger, "prioritize_unread_books", e)
        return UnreadPriorityResponse(
            high_priority=[],
            medium_priority=[],
            low_priority=[],
    ...
    try:
        ...
        for update_request in updates:
            try:
                ...
                log_operation(logger, "book_metadata_update", level="INFO",
                             book_id=book_id, updated_fields=list(updates_dict.keys()), status="success")
        ...
        log_error(logger, "update_metadata", e)
        return MetadataUpdateResponse(
            updated_books=[],
...
async def auto_organize_tags() -> TagStatsResponse:
    ...
    try:
        ...
        log_error(logger, "ai_tag_organization", e)
...
async def fix_metadata_issues() -> MetadataUpdateResponse:
    ...
    try:
        ...
        for book_id, fix_description in common_fixes:
            ...
            log_operation(logger, "book_fix", level="INFO",
                         book_id=book_id, fix_description=fix_description, status="success")
        ...
        log_error(logger, "fix_metadata_issues", e)
        return MetadataUpdateResponse(
            updated_books=[],
            failed_updates=[(0, str(e))],
            total_processed=0,
...
async def reading_statistics() -> ReadingStats:
    ...
    try:
        ...
        log_error(logger, "generate_reading_statistics", e)
        return ReadingStats(
            total_books_owned=0,
            books_read=0,
            books_unread=0,
...
async def convert_book_format(
    ...
    try:
        ...
        for request in conversion_requests:
            ...
            log_operation(logger, "book_conversion", level="INFO",
                         book_id=book_id, target_format=target_format, status="success")
        
        return results
        
    except Exception as e:
        log_error(logger, "convert_books", e)
...
async def download_book(
    ...
    try:
        ...
        log_error(logger, "prepare_download", e)
        return {
            "success": False,
            "error": str(e),
            "book_id": book_id
...
async def bulk_format_operations(
    ...
    try:
        ...
        log_error(logger, "bulk_format_operations", e)
        return {
            "error": str(e),
...
async def japanese_book_organizer() -> JapaneseBookOrganization:
    ...
    """
    try:
        log_operation(logger, "weeb_mode_activation", level="INFO",
                     mode="Japanese library optimization", status="activated")
        ...
        log_error(logger, "japanese_book_organization", e)
        return JapaneseBookOrganization(
            manga_series=[],
            light_novels=[],
            visual_novels=[],
...
async def it_book_curator() -> ITBookCuration:
    ...
    """
    try:
        log_operation(logger, "it_curation_mode_activation", level="INFO",
                     mode="Technology book optimization", status="activated")
        ...
        log_error(logger, "it_book_curation", e)
        return ITBookCuration(
            programming_languages={},
            outdated_books=[],
            trending_technologies=[],
...
async def reading_recommendations() -> ReadingRecommendations:
    ...
    """
    try:
        log_operation(logger, "austrian_efficiency_mode_activation", level="INFO",
                     mode="Smart reading recommendations", status="activated")
        ...
        log_error(logger, "reading_recommendations", e)
        return ReadingRecommendations(
            next_in_series=[],
            similar_to_liked=[],
            trending_in_library=[],
...
def main():
    ...
    log_operation(
        ...
        "server_startup",
        level="INFO",
        server_name="CalibreMCP Phase 2",
        version="FastMCP 2.0",
        features=["weeb_optimization", "23_comprehensive_tools"],
        collection_size="1000+ books"
    )
    
    # Log library discovery
    log_operation(
        logger,
        "library_discovery",
        level="INFO",
        discovered_libraries=["Main", "IT", "Japanese", "Academic"],
        current_library=current_library
    )
