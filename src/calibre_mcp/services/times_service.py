...
class TimesService:
    ...
    def stats_by_month_added(self, year: Optional[int] = None) -> Dict[str, Any]:
        ...
        with self._get_session() as session:
            ...
            rows = q.all()
            items = [
                {"year": int(r.yr) if r.yr else None, "month": int(r.mo) if r.mo else None, "count": r.count}
                for r in rows
            ]
            return {"by_month": items, "total_books": sum(r.count for r in rows)}

    def stats_by_month_published(self, year: Optional[int] = None) -> Dict[str, Any]:
    ...
    def date_stats(self) -> Dict[str, Any]:
        ...
        with self._get_session() as session:
            ...
            ).filter(Book.timestamp.isnot(None)).first()
            pd = session.query(
                func.min(Book.pubdate).label("min_pub"),
                func.max(Book.pubdate).label("max_pub"),
            ).filter(Book.pubdate.isnot(None)).first()
            return {
                "added_date_range": {
