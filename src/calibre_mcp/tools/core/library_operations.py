...
async def search_books(
    ...
    """
    Search books using text queries across multiple fields.
    
    Performs full-text search across specified fields with boolean operators.
    Supports searching in title, author, tags, comments, and other metadata.
    
    Args:
        text: Search query text
        fields: List of fields to search (title, author, tags, etc.)
        operator: Boolean operator for multi-field searches (AND/OR)
        
    Returns:
        LibrarySearchResponse: Search results with relevance scoring
    """
    try:
        start_time = asyncio.get_event_loop().time()
        client = await get_api_client()
        ...
        results = await client.advanced_search(
            text=text,
            fields=fields or ['title', 'author', 'tags'],
            operator=operator
        )
        ...
        log_error(logger, "search_books", e)
        return LibrarySearchResponse(
            results=[],
            total_found=0,
            query_used=text,
