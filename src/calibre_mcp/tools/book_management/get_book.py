...
async def get_book_helper(
    ...
    try:
        ...
        if not client:
            # Local SQLite mode: make sure the DB singleton is initialized before use
            db_err = ensure_db_initialized()
            if db_err:
                raise MCPServerError(db_err)
        if client:
            ...
            result.update(
                {
                    "publisher": book_data.get("publisher"),
                    "pubdate": book_data.get("pubdate"),
                    "series": book_data.get("series", {}).get("name")
                    if isinstance(book_data.get("series"), dict)
