...
import { type Book, getBookCoverUrl, ratingToFiveStarCount } from '@/common/api';
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {book.tags &&
            book.tags.slice(0, 5).map((tag, i) => {
              const label =
                typeof tag === 'string' ? tag : ((tag as { name?: string }).name ?? String(tag));
              return (
                <span
                  key={`${book.id}-tag-${i}`}
                  className="text-[10px] bg-slate-700/50 px-2 py-0.5 rounded text-slate-300 border border-slate-600/50"
                >
                  {label}
                </span>
              );
