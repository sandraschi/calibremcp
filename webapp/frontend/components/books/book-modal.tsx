...
export function BookModal({ book, onClose }: BookModalProps) {
  ...
  const [showEmbeddedReader, setShowEmbeddedReader] = useState(false);
  ...
  const handleFetchMetadata = async () => {
    ...
    try {
      const res = await fetchBookMetadataOnline(book.id, {
        includeCover: true,
      });
      if (!res.success) {
  ...
  return (
    ...
    >
            ...
            <div className="sticky top-0">
              ...
              <div className="mt-8 flex flex-col gap-3">
                ...
                >
                  ...
                  {readLoading ? 'Opening...' : 'Read book (Desktop)'}
                </button>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setShowEmbeddedReader(true)}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-amber/15 text-amber border border-amber/50 font-semibold rounded-md hover:bg-amber/25 transition-colors shadow text-xs"
                    title="Open reader inside this window"
                  >
                    <Eye className="w-4 h-4 text-amber shrink-0" />
                    Read Here
                  </button>
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-slate-800 text-amber font-semibold rounded-md hover:bg-slate-700 border border-amber/40 transition-colors shadow text-xs"
                    title="Open in new browser tab (:8099)"
                  >
                ...
                {!loading && (
                  <>
                    ...
                    {fetchMetaError && (
                      <p className="text-xs text-red-400 text-center leading-snug">
                        {fetchMetaError}
                      </p>
                    )}
                    {fetchMetaMessage && !fetchMetaError && (
                      <p className="text-xs text-emerald-400/90 text-center leading-snug">
                        {fetchMetaMessage}
                      </p>
                    )}
                    <p className="text-[10px] text-slate-500 text-center leading-snug">
                      Uses Calibre&apos;s tools on PATH. Close the main Calibre app if the library
                      is locked.
                    </p>
                  </>
                )}
                  ...
                  <div className="pt-4 border-t border-slate-700 space-y-4">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 mb-2 leading-snug">
                        Notable works may have Wikipedia articles; guessed links can 404. Search
                        links always work.
                        {isTvTropesFleetMirrorEnabled() ? (
                          <span className="block mt-1 text-emerald-500/90">
                            TV Tropes opens in fleet mirror{' '}
                            <code className="text-[9px]">tvtropes-mcp</code> (port 10965) when
                            running; Google site search if the page is not cached yet.
                          </span>
                        ) : isTvTropesViaConfigured() ? (
                          <span className="block mt-1 text-emerald-500/90">
                            TV Tropes live links use{' '}
                            <code className="text-[9px]">NEXT_PUBLIC_TVTROPES_VIA</code> (legacy
                            proxy).
                          </span>
                        ) : (
                          <span className="block mt-1">
                            Start <code className="text-[9px]">tvtropes-mcp</code> for cached TV
                            Tropes pages, or use Google site search. Optional:{' '}
                      ...
                      <div className="flex flex-col gap-1.5">
                        {externalLinks.length === 0 ? (
                          <p className="text-xs text-slate-500 italic">
                            Add ISBN or a Wikipedia identifier in Calibre for stronger matches.
                            Wikipedia / Wikidata search still works from title and author.
                          </p>
                        ) : (
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 mb-2 leading-snug">
                        Opens RAG with this book prefilled. Run the action there (can take 10–30s
                        for research).
                      </p>
                      <div className="flex flex-col gap-1.5">
                        <Link
          ...
          <div className="flex-1 min-w-[300px] p-6 border-b md:border-b-0 md:border-r border-slate-700 bg-slate-800/20">
            ...
            {loading ? (
              ...
              <div className="mt-6 space-y-6">
                <div>
                  <h3 className="text-xs font-uppercase tracking-wider text-slate-500 font-bold mb-2 uppercase">
                    Core Details
                  </h3>
                  <dl className="grid grid-cols-1 gap-x-4 gap-y-2 text-sm">
                        ...
                        <dd className="text-amber font-mono tracking-widest text-xs">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span
                              key={i}
                              className={
                                i < ratingToFiveStarCount(displayBook.rating)
                                  ? 'text-amber'
                                  : 'text-slate-600'
                              }
                            >
                    ...
                    {seriesName && (
                      <div className="flex flex-col border-b border-slate-700/50 pb-1">
                        ...
                        <dd className="text-slate-200">
                          ...
                          {displayBook.series_index != null ? ` [${displayBook.series_index}]` : ''}
                        </dd>
                      </div>
                    )}
                    ...
                    {displayBook.pubdate && (
                      <div className="flex justify-between border-b border-slate-700/50 pb-1">
                        ...
                        <dd className="text-slate-200">
                          {String(displayBook.pubdate).slice(0, 10)}
                        </dd>
                      </div>
                    )}
                  </dl>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {formats.map((f) => (
                        <span
                          key={f}
                          className="text-xs font-mono bg-amber/10 border border-amber/30 text-amber px-2 py-0.5 rounded"
                        >
      ...
      {/* Embedded Reader Overlay */}
      {showEmbeddedReader && (
        <div
          className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex flex-col p-2 sm:p-6"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl flex flex-col flex-1 overflow-hidden">
            {/* Header bar */}
            <div className="h-12 px-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                <span className="text-xs text-slate-500 hidden sm:inline">
                  — Calibre In-Browser Reader (:8099)
                </span>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-2.5 py-1 text-xs text-amber hover:bg-slate-800 rounded border border-amber/30 flex items-center gap-1 transition-colors"
                ...
                <button
                  type="button"
                  onClick={() => setShowEmbeddedReader(false)}
            ...
            <div className="flex-1 w-full bg-slate-950 relative">
              <iframe
                src={getBookReaderUrl(book.id)}
                title={`Reader - ${displayBook.title}`}
