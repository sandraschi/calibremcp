...
export function LoggerModal({ onClose }: LoggerModalProps) {
  ...
  const filteredLines = lines.filter((line) => levelRank(lineLevel(line)) <= levelRank(minLevel));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <div className="px-4 py-2 text-sm text-red-400 border-b border-slate-600">{error}</div>
        )}
