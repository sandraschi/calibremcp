...
import { type Library, switchLibrary } from "@/common/api";
	...
	const handleSwitchLibrary = async () => {
		...
		try {
			...
			if (result.success) {
				...
				setMessage({
					type: "error",
					text: result.message || "Failed to switch library",
				});
			}
		} catch (error) {
			setMessage({
			...
			<div className="space-y-4">
				...
				<div className="border-t border-slate-600 pt-4">
					...
					<ul className="space-y-1 text-sm text-slate-400">
						<li>
							• Click a library card to switch (Browse and Search use active
							library)
						</li>
					...
					<p className="text-xs text-slate-500 mt-2">
						Note: Library backup and CRUD operations will be available in a
						future update.
					</p>
				</div>
			</div>
