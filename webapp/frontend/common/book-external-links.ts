...
export function isTvTropesViaConfigured(): boolean {
  return Boolean(typeof process !== 'undefined' && process.env.NEXT_PUBLIC_TVTROPES_VIA?.trim());
}

function lowerKeyMap(id: Record<string, string> | undefined): Record<string, string> {
  if (!id) return {};
...
export function buildBookExternalLinks(book: {
  ...
  const wikiId =
    id.wikipedia ?? id.wiki ?? id['wikipedia-en'] ?? id['en.wikipedia'] ?? id.ewikipedia;
  if (wikiId) {
  ...
  const rawIsbn = id.isbn13 ?? id.isbn10 ?? id.isbn ?? id['isbn-13'] ?? id['isbn-10'];
  const isbn = rawIsbn ? normIsbn(String(rawIsbn)) : '';

  if (isbn && /^[\dX]{10,13}$/i.test(isbn)) {
    push('Open Library (ISBN)', `https://openlibrary.org/isbn/${encodeURIComponent(isbn)}#book`);
    push('WorldCat', `https://www.worldcat.org/search?q=isbn%3A${encodeURIComponent(isbn)}`);
    push(
      'Google Books',
      `https://www.google.com/search?tbm=bks&q=${encodeURIComponent(`isbn:${isbn}`)}`,
    );
    push('LibraryThing', `https://www.librarything.com/isbn/${encodeURIComponent(isbn)}`);
    push(
  ...
  const ol = id.openlibrary ?? id.ol ?? id['openlibrary-work'] ?? id['openlibrary-edition'];
  if (ol) {
    const s = String(ol).trim();
    if (/^OL\d+W$/i.test(s)) {
      push('Open Library (work)', `https://openlibrary.org/works/${encodeURIComponent(s)}`);
    } else if (/^OL\d+M$/i.test(s)) {
      push('Open Library (edition)', `https://openlibrary.org/books/${encodeURIComponent(s)}`);
    } else {
      push('Open Library (search)', `https://openlibrary.org/search?q=${encodeURIComponent(s)}`);
    }
  }
  ...
  const doi = id.doi;
  if (doi) push('DOI', `https://doi.org/${encodeURIComponent(String(doi).trim())}`);

  const lccn = id.lccn;
  if (lccn) {
    const clean = String(lccn).replace(/\s+/g, '');
    if (clean) push('Library of Congress', `https://lccn.loc.gov/${encodeURIComponent(clean)}`);
  }
