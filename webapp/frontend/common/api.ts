...
export async function setAnnasMirrors(
...
export async function fetchBookMetadataOnline(
	...
	const response = await fetch(
		`${getBaseUrl()}/api/books/${bookId}/fetch-metadata`,
		{
			method: "POST",
	...
	const data = (await response
		.json()
		.catch(() => ({}))) as FetchBookMetadataOnlineResult & {
...
export async function getLibraryStats(
	...
	if (!response.ok) {
		...
		throw new Error(
			detail || `Failed to fetch library stats (${response.status})`,
		);
	}
	const data = await response.json();
...
function normalizeLibraryStats(data: Record<string, unknown>): LibraryStats {
	return {
		library_name:
			(data.library_name as string) ?? (data.library as string) ?? "Unknown",
		total_books: (data.total_books as number) ?? (data.books as number) ?? 0,
		total_authors:
			(data.total_authors as number) ?? (data.authors as number) ?? 0,
		...
		format_distribution:
			(data.format_distribution as Record<string, number>) ?? {},
		language_distribution:
			(data.language_distribution as Record<string, number>) ?? {},
		rating_distribution:
...
export async function getSystemStatus(
	...
	const response = await fetch(
		`${base}/api/system/status?status_level=${level}`,
	);
...
export async function listPublishers(params?: {
	...
	if (params?.offset) searchParams.set("offset", params.offset.toString());
	const response = await fetch(
		`${getBaseUrl()}/api/publishers?${searchParams}`,
	);
	if (!response.ok) {
		let detail = "";
		try {
...
export async function ragMetadataBuild(
	forceRebuild = false,
): Promise<RagMetadataBuildResult> {
	const response = await fetch(
...
export async function ragMetadataSearch(
	...
	const response = await fetch(
		`${getBaseUrl()}/api/rag/metadata/search?${params}`,
	);
	if (!response.ok) {
		const err = await response.json().catch(() => ({}));
		throw new Error(
...
export async function ragContentBuild(
	forceRebuild = false,
): Promise<RagMetadataBuildResult> {
	const response = await fetch(
...
export async function ragSynopsis(
	bookId: number,
	spoilers = false,
): Promise<RagSynopsisResult> {
	const response = await fetch(
...
export async function getSeriesAnalysis(
	seriesName: string,
