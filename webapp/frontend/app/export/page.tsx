		...
		try {
			...
			if (res.ok && data.success !== false) {
				setResult({
					...
					message:
						data.message ??
						`Exported ${data.count ?? "books"} to ${format.toUpperCase()}`,
				});
			} else {
				setResult({
					success: false,
	...
	return (
		<div className="container mx-auto p-6">
			...
			<p className="text-slate-400 mb-6">
				Export books to CSV or JSON. Filter by author or tag. Output is returned
				from the server.
			</p>
			<form onSubmit={handleSubmit} className="max-w-xl space-y-4">
				...
				<div>
					...
					<input
						...
						value={limit}
						onChange={(e) =>
							setLimit(Number.parseInt(e.target.value, 10) || 1000)
						}
						className="w-full px-4 py-2 rounded-lg bg-slate-800 border border-slate-600 text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber"
					/>
				</div>
