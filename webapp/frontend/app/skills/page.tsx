...
function SkillsPageInner() {
  const [skills, setSkills] = useState<
    { id: string; name: string; prompt: string; resource?: string }[]
  >([]);
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <p className="text-slate-400 mb-6">
        Bundled expert instructions are exposed over MCP as{' '}
        <code className="text-amber/90">skill://</code> resources (FastMCP 3.1). Related prompts
        guide Chat; use an MCP client to fetch the full SKILL.md when needed.
      </p>
      ...
      <ul className="grid gap-3 max-w-2xl">
        {skills.map((s) => (
          ...
          >
            ...
            <Link href="/chat" className="text-sm text-amber hover:underline">
              Use in Chat
            </Link>
          </li>
        ))}
      ...
      <p className="mt-6 text-slate-500 text-sm">
        For agentic tool chaining and sampling, see the{' '}
        <Link href="/agentic" className="text-amber hover:underline">
          Agentic
        </Link>{' '}
        page.
      </p>
