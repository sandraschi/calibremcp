	...
	const handleAnnasImport = async (md5: string, title: string) => {
		...
		try {
			...
			if (res.success) {
				...
				showGlobalResult(
					false,
					"Mirror restricted: This library requires manual download (e.g. CAPTCHA).",
				);
			} else {
	...
	return (
		<div className="container mx-auto p-6 max-w-6xl">
			<div className="mb-8 flex justify-between items-center">
				...
				{globalResult && (
					<div
						className={`px-4 py-2 rounded-lg text-sm font-bold shadow-lg animate-in fade-in slide-in-from-top-4 duration-300 ${
				...
				<div className="ml-auto flex items-center gap-4">
					<button
						...
						className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all flex items-center gap-2 ${
							showOptions
								? "bg-amber/10 border-amber text-amber shadow-[0_0_15px_rgba(245,158,11,0.1)]"
					...
					<div className="animate-in fade-in slide-in-from-left-4 duration-300">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{annasResults.map((book) => (
								<div
									key={book.detail_item}
									className="p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:border-amber/30 transition-all group"
								>
									<div className="flex justify-between items-start mb-2">
										<h3 className="font-bold text-slate-100 line-clamp-1 group-hover:text-amber transition-colors">
											{book.title}
										</h3>
										<span className="text-[10px] bg-slate-700 px-2 py-0.5 rounded text-slate-400 uppercase tracking-widest">
											{book.formats}
										</span>
									</div>
									...
									<div className="flex flex-col gap-2">
										{restrictedMd5s[book.detail_item] && (
											<div className="text-[10px] text-amber-500 font-bold bg-amber-500/10 p-2 rounded border border-amber-500/20 mb-1">
												Landers found. Please use source page to handle
												CAPTCHA/timers manually.
											</div>
										)}
										<div className="flex gap-2">
											<button
												onClick={() =>
													handleAnnasImport(book.detail_item, book.title)
												}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{gutenResults.map((book) => (
								<div
									...
									className="p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:border-amber/30 transition-all group"
								>
									<h3 className="font-bold text-slate-100 line-clamp-1 mb-1 group-hover:text-amber transition-colors">
										{book.title}
									</h3>
									<p className="text-sm text-slate-400 mb-4 line-clamp-1">
										{book.authors.join(", ")}
					...
					<div className="animate-in fade-in slide-in-from-left-4 duration-300">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							{arxivResults.map((paper) => (
								<div
									key={paper.id}
									className="p-5 bg-slate-800/40 border border-slate-700/50 rounded-2xl hover:border-amber/30 transition-all flex flex-col"
								>
									<div className="flex-1">
										<h3 className="font-bold text-slate-100 mb-2 line-clamp-2 leading-snug">
											{paper.title}
										</h3>
										<p className="text-xs text-amber/80 font-medium mb-3 uppercase tracking-wider">
