...
export default function ChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const systemContent = buildSystemPrompt(skillContent, preprompt, contextInfo);
    const messagesToSend = systemContent
      ? [{ role: 'system' as const, content: systemContent }, ...messages, userMsg]
      : [...messages, userMsg];
    try {
  ...
  const handleExport = () => {
    const lines = messages.map(
      (m) => `[${m.ts || 'unknown'}] ${m.role === 'user' ? 'You' : 'Assistant'}: ${m.content}`,
    );
  ...
  return (
    <div className="mx-auto p-6 flex flex-col flex-1 min-h-0" data-testid="chat-page">
      {/* Controls bar */}
          ...
          <div className="self-stretch flex-1 max-w-md">
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                try {
                  localStorage.setItem('calibre-mcp-chat-custom-prompt', e.target.value);
                } catch {}
              }}
          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[85%] rounded-lg px-4 py-2 ${
                msg.role === 'user' ? 'bg-amber/20 text-zinc-200' : 'bg-zinc-700 text-zinc-200'
              }`}
            >
