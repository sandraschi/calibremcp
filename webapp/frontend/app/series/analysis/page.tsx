...
import {
	type SeriesAnalysisBook,
	type SeriesAnalysisResult,
	getSeriesAnalysis,
} from "@/common/api";
...
export default function SeriesAnalysisPage() {
	...
	const owned =
		result?.owned_count ?? result?.books?.filter((b) => b.owned).length ?? 0;
	const total = result?.total_volumes ?? result?.books?.length ?? 0;
	const missing =
		result?.missing ??
		result?.books?.filter((b) => !b.owned).map((b) => b.title) ??
		[];
		...
		<div className="container mx-auto p-6 space-y-6 max-w-2xl">
			...
			<div>
				...
				<p className="text-slate-400 mt-1">
					Reading order, completion status, and missing volumes for any series
					in your library.
				</p>
			</div>
			...
			<div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 space-y-3">
				...
				<p className="text-xs text-slate-500">
					Series name must match how it appears in Calibre (partial match may
					work depending on backend).
				</p>
			</div>
