...
export default function SeriesAnalysisPage() {
  ...
  const missing =
    result?.missing ?? result?.books?.filter((b) => !b.owned).map((b) => b.title) ?? [];

  return (
    <div className="container mx-auto p-6 space-y-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-2">
        <Link href="/series" className="text-slate-400 hover:text-slate-200 text-sm">
          ← Series
        </Link>
      </div>
      ...
      <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Series name must match how it appears in Calibre (partial match may work depending on
          backend).
        </p>
      </div>
