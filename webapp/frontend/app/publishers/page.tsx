...
import { listPublishers } from '@/common/api';
...
function PublishersPageInner() {
  ...
  const [data, setData] = useState<{ items: { id: number | null; name: string; book_count?: number }[]; total: number } | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    listPublishers({ query, limit, offset })
      .then((res) => {
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-6">
        <p className="text-slate-400">Loading publishers…</p>
      </div>
    );
  }
      ...
      {total > limit && (
        <nav className="mt-6 flex flex-wrap items-center justify-center gap-2" aria-label="Pagination">
          ...
          <div className="flex items-center gap-2">
            {hasPrev ? (
              <Link
                href={`/publishers?page=${page - 1}${query ? `&query=${encodeURIComponent(query)}` : ''}`}
