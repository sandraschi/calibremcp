...
export default function RagPage() {
  ...
  async function handleBuild(type: 'metadata' | 'content', force: boolean) {
    ...
    try {
      const res =
        type === 'metadata' ? await ragMetadataBuild(force) : await ragContentBuild(force);
        ...
        const poll = async () => {
          try {
            ...
            if (status.status === 'done') {
              stopPolling();
              setBuildMessage(
                status.total ? `Indexed ${status.total} books.` : status.message || 'Done.',
              );
    ...
    <div className="container mx-auto p-6 space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-slate-400 mt-1">
          Semantic search over your library using LanceDB. Metadata search works on titles, authors,
          tags and comments. Passage search works on full book text. Build the relevant index before
          first use.
        </p>
      </div>
        ...
        {(['metadata', 'passages', 'synopsis', 'research'] as SearchMode[]).map((m) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors capitalize ${
              mode === m ? 'bg-amber/20 text-amber' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {m === 'metadata'
              ? 'Metadata'
        ...
        {mode === 'synopsis' || mode === 'research' ? (
          <div className="space-y-3">
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1">Find the ID in the book URL: /book/1234</p>
            </div>
          ...
          {metaResults.map((hit, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                {hit.text && <p className="text-sm text-slate-400 mt-1 line-clamp-2">{hit.text}</p>}
                {hit.score !== undefined && (
        ...
        <div className="space-y-3">
          ...
          {researchResult.local_data && (
            <div className="flex flex-wrap gap-2 text-xs text-slate-400">
              {researchResult.local_data.rating != null && researchResult.local_data.rating > 0 && (
                <span>
                  Your rating: {'★'.repeat(ratingToFiveStarCount(researchResult.local_data.rating))}
