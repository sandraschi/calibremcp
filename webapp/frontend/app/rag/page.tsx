...
export default function RagPage() {
	...
	const [researchResult, setResearchResult] =
		useState<RagResearchResult | null>(null);
		...
		try {
			if (mode === "metadata") {
				...
				if (res.error) setError(res.error);
					...
					setMessage("No results — is the metadata index built?");
				...
				if (res.error) setError(res.error);
					...
					setMessage("No results — is the content index built?");
		...
		<div className="container mx-auto p-6 space-y-6 max-w-3xl">
			<div>
				...
				<p className="text-slate-400 mt-1">
					Semantic search over your library using LanceDB. Metadata search works
					on titles, authors, tags and comments. Passage search works on full
					book text. Build the relevant index before first use.
				</p>
			</div>
			...
			<SectionBox title="Indexes">
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						<p className="text-sm text-slate-400 mb-2">
							<span className="text-slate-200 font-medium">Metadata index</span>{" "}
							— titles, authors, tags, comments. Fast to build (~seconds).
						</p>
						<div className="flex gap-2">
							<button
								type="button"
								onClick={() => handleBuild("metadata", false)}
							...
							<button
								type="button"
								onClick={() => handleBuild("metadata", true)}
								disabled={buildingMeta || buildingContent}
					...
					<div>
						<p className="text-sm text-slate-400 mb-2">
							<span className="text-slate-200 font-medium">Content index</span>{" "}
							— full book text. Slow to build (minutes for large libraries).
						</p>
						<div className="flex gap-2">
							<button
								type="button"
								onClick={() => handleBuild("content", false)}
								disabled={buildingMeta || buildingContent}
								className="px-3 py-1.5 rounded-lg bg-amber/20 text-amber text-sm font-medium hover:bg-amber/30 disabled:opacity-50"
							>
							...
							<button
								type="button"
								onClick={() => handleBuild("content", true)}
								disabled={buildingMeta || buildingContent}
								className="px-3 py-1.5 rounded-lg bg-slate-600 text-slate-300 text-sm hover:bg-slate-500 disabled:opacity-50"
							>
								Rebuild
				...
				{(buildingMeta || buildingContent) && buildStatus && (
					<BuildProgress status={buildStatus} />
				)}
				{buildMessage && (
					<p className="mt-2 text-sm text-slate-300">{buildMessage}</p>
				)}
				{buildError && (
					<p className="mt-2 text-sm text-red-300">{buildError}</p>
				)}
			</SectionBox>

			{/* ── Mode tabs ── */}
			<div className="flex gap-1 border border-slate-600 rounded-lg p-1 bg-slate-800/50 w-fit">
				{(["metadata", "passages", "synopsis", "research"] as SearchMode[]).map(
					(m) => (
						<button
							key={m}
							type="button"
					...
					<div className="space-y-3">
						...
						{mode === "research" && (
							<p className="text-xs text-slate-500">
								Fetches Wikipedia, SF Encyclopedia, TVTropes, Open Library +
								your Calibre data. Takes 10–30 seconds. Requires Claude Desktop
								/ Cursor (LLM sampling).
							</p>
						)}
					</div>
			...
			{mode === "metadata" && metaResults.length > 0 && (
				<ul className="space-y-2">
					{metaResults.map((hit, i) => (
						...
						>
							<span className="text-xs text-slate-500 mt-0.5 w-5 shrink-0 text-right">
								{i + 1}
							</span>
							<div className="min-w-0">
								<Link
									href={`/book/${hit.book_id}`}
			...
			{mode === "passages" && passageResults.length > 0 && (
				<ul className="space-y-3">
					{passageResults.map((hit, i) => (
						<li
							key={`${hit.book_id ?? hit.arxiv_id}-${hit.chunk_idx}-${i}`}
			...
			{mode === "synopsis" && synopsis && (
				<div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4">
					...
					{spoilers && (
						<p className="text-xs text-amber/60 mt-3">
					...
					{researchResult.local_data && (
						<div className="flex flex-wrap gap-2 text-xs text-slate-400">
							{researchResult.local_data.rating != null &&
								researchResult.local_data.rating > 0 && (
									<span>
										Your rating:{" "}
										{"★".repeat(
											ratingToFiveStarCount(researchResult.local_data.rating),
										)}
										{"☆".repeat(
											5 -
												ratingToFiveStarCount(researchResult.local_data.rating),
							...
							{(researchResult.local_data.rag_passages ?? 0) > 0 && (
								<span>
									{researchResult.local_data.rag_passages} RAG passages included
								</span>
							)}
						</div>
					)}
