...
export default async function AuthorsPage({
  ...
  try {
    data = await listAuthors({ query: params.query, limit, offset });
  } catch (e) {
    return (
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6 text-slate-100">Authors</h1>
      ...
      {total > limit && (
        <nav className="mt-6 flex flex-wrap items-center justify-center gap-2" aria-label="Pagination">
          ...
          <div className="flex items-center gap-2">
            {hasPrev ? (
              <Link
                href={`/authors?page=${page - 1}${params.query ? `&query=${encodeURIComponent(params.query)}` : ''}`}
