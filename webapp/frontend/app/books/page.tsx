...
import { type BookListResponse, getBooks } from "@/common/api";
...
const BACKEND_HINT =
	"From repo root run webapp\\start.ps1 (backend 10720, frontend 10721).";
			...
			{total > limit && (
				...
				>
					<p className="text-sm text-slate-400">
						Showing{" "}
						<span className="text-slate-200 font-medium">{offset + 1}</span>–
						<span className="text-slate-200 font-medium">
							{Math.min(offset + limit, total)}
						</span>{" "}
						of <span className="text-slate-200 font-medium">{total}</span> books
					</p>
