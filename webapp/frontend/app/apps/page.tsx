        ...
        setLaunchTarget((t) =>
          t
            ? {
                ...t,
                status: 'error',
                error: (data.detail ?? data.error ?? `HTTP ${r.status}`) as string,
              }
            : null,
        );
        return;
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <p className="text-slate-400 mb-8 max-w-2xl">
        Fleet webapps and containers discovered by probing known ports. Green = reachable, red =
        down.
      </p>

      <section className="mb-10">
        <h2 className="text-xl font-semibold text-slate-200 mb-4">Webapps ({apps.length})</h2>
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <h3 className="font-semibold text-slate-100 text-sm truncate">{app.label}</h3>
                  </div>
                  <span className="text-slate-500 text-xs shrink-0">:{app.port}</span>
                </div>
                <p className="text-slate-400 text-xs mb-3 flex-1">{app.description}</p>
                <button
                  type="button"
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <h3 className="font-semibold text-slate-100 text-sm truncate">{c.label}</h3>
                </div>
                <span className="text-slate-500 text-xs shrink-0">:{c.port}</span>
              </div>
