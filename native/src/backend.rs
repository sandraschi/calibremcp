...
const BACKEND_NAME: &str = "calibre-mcp-backend.exe";
const BACKEND_PORT: u16 = 10720;
const BACKEND_TAG: &str = "calibre-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "CALIBRE_TAURI";
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("calibre-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/calibre-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name calibre-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM calibre-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    if !free_port(BACKEND_PORT) {
        let msg = format!(
            "Could not free port {BACKEND_PORT} after 240s -- TIME_WAIT not cleared"
        );
        log_line(&app, &msg);
        return Err(msg);
    }
